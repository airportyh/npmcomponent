...
function mutateModule(repo, callback){
  ...
  fs.readFile(path.join(repo.dirpath(), 'component.json'), function(err, data){
    ...
    if (config.scripts && !is.array(config.scripts)){
      log.error(repo, '"scripts" is not an array')
      return callback(new Error(repo + ': "scripts" is not an array'))
    }
    var jsFiles = (config.scripts || []).map(function(script){
      return path.join(repo.dirpath(), script)
    })
    if (config.templates && !is.array(config.templates)){
      log.error(repo, '"templates" is not an array')
      return callback(new Error(repo + ': "templates" is not an array'))
    }
  ...
  fs.readdir(repo.dirpath(), function(err, entries){
    ...
    fs.readFile(readmePath, function(err, buf){
      ...
      var msg = '*This repository is a mirror of the [component](http://component.io) module [' + originalName + '](' + upstreamUrl + '). ' +
        ...
        'Please do not open issues or send pull requests against this repo. ' + 
        'If you have issues with this repo, report it to [npm-component](https://github.com/airportyh/npm-component).' +
        '*'
      text = msg + '\n' + text
          ...
          if (requirePath in dep){
            ...
            if (requirePath[0] === '.'){
              ...
              if (node.parent.type === 'VariableDeclarator' &&
                ...
                var varDecl = node.parent.parent
                varDecl.update('// commented out by npm-component: ' + varDecl.source())
                log.info(repo, 'commented out require(' + requirePath + ')')
              }else{
                log.warn(repo, 'require not found: ' + requirePath + ' and no action taken')
              }
            }
          }
        }
