  ...
  fs.readFile(path.join(dirpath, 'component.json'), function(err, data){
    ...
    glob(path.join(dirpath, '**/*.js'), function(err, files){
      async.each(files, function(filepath, next){
        ...
        writePackageJson(dirpath, moduleName, config, deps, function(err){
          if (err) return callback(err)
...
function modifyReadMe(dirpath, moduleName, callback){
  ...
  fs.readdir(dirpath, function(err, entries){
    ...
    fs.readFile(readmePath, function(err, buf){
      var text = buf + ''
      var msg = '*This repository is a mirror of the <http://component.io> module <http://github.com/' + 
        moduleName.replace('-', '/') + '>. It has been modified to work with NPM+Browserify. ' +
